...
from security.blacklist import is_token_blacklisted, is_user_session_revoked
...
async def get_auth_context(
    ...
    # Verifica se as sessões do usuário foram invalidadas (ex: alteração de senha)
    token_iat = payload.get("iat")
    if await is_user_session_revoked(user_id, token_iat):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Sessão encerrada por alteração de senha. Faça login novamente.",
        )
