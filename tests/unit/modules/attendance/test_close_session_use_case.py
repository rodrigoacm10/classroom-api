...
class TestCloseAttendanceSessionUseCase:
    ...
    async def test_close_session_success(self):
        ...
        use_case = make_use_case(session_repo, subject_class_repo, tenant_repo, member_repo)
    ...
    async def test_close_session_publishes_closed_event(self):
        """Deve publicar AttendanceSessionClosedEvent com os dados corretos após encerrar sessão."""
        session_repo = FakeAttendanceSessionRepository()
        subject_class_repo = FakeSubjectClassRepository()
        tenant_repo = FakeTenantRepository()
        member_repo = FakeTenantMemberRepository()
        ...
        assert isinstance(event, AttendanceSessionClosedEvent)
        assert event.session_id == closed.id
